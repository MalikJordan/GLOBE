...
def bgc_rate_eqns(iter, base_element, parameters, tracers):
    ...
    for key in tracers:
        if tracers[key].type == "bacteria":
            ...
            tracers[key].phyto(iter, base_element, environmental_parameters["base_temp"], water_column_parameters["z"], water_column_parameters["dz"], mixed_layer_depth, surface_PAR, temperature, tracers)
...
def bgc_rate_eqns_solveivp(time, base_element, parameters, tracers):
